...
export function MoviesPage({ state }: Props) {
  ...
  const licensedMovies = state?.currentMovies ?? [];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-slate-900/60 border border-slate-800/50 rounded-xl px-4 py-3">
          ...
          <div className="text-lg font-bold text-green-400">{allMovies.filter(m => m.minReputation <= currentRep).length}</div>
        </div>
        <div className="bg-slate-900/60 border border-slate-800/50 rounded-xl px-4 py-3">
        ...
        <div className="bg-slate-900/60 border border-slate-800/50 rounded-xl px-4 py-3">
          ...
          <div className="text-lg font-bold text-amber-400">${Math.round(allMovies.reduce((s, m) => s + m.licenseCost, 0) / allMovies.length)}/wk</div>
        </div>
      </div>
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-slate-600 text-left border-b border-slate-800/40">
                    ...
                    <th className="py-2 pr-3 font-medium text-right">License/wk</th>
                    <th className="py-2 pr-3 font-medium text-right">Min Rep</th>
                    <th className="py-2 font-medium text-center">Status</th>
                  </tr>
                </thead>
                <tbody>
                  {movies.sort((a, b) => a.minReputation - b.minReputation).map(movie => {
                    const unlocked = currentRep >= movie.minReputation;
                    const licensed = licensedMovies.includes(movie.id);
                    return (
                      <tr key={movie.id} className={`border-b border-slate-800/20 ${!unlocked ? 'opacity-40' : ''}`}>
                        ...
                        <td className="py-2 text-center">
                          {licensed ? (
                            <span className="text-[9px] bg-green-500/20 text-green-400 px-1.5 py-0.5 rounded">LICENSED</span>
                          ) : unlocked ? (
